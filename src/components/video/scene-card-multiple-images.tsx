...
export function SceneCardMultipleImages({ scene, sceneIndex, artStyle, aspectRatio, tone, onDelete, onUpdate }: SceneCardProps) {
  ...
  const handleGenerateImage = async (imageType: 'start' | 'end') => {
    ...
    try {
      ...
      if (imageType === 'start') {
        onUpdate(sceneIndex, { ...scene, startImageUrl: imageDataUri });
      } else {
        onUpdate(sceneIndex, { ...scene, endImageUrl: imageDataUri });
      }
    } catch (error) {
  ...
  const startImageSrc = scene.startImageUrl || `https://picsum.photos/seed/${sceneIndex + 1}/720/1280`;
  const endImageSrc = scene.endImageUrl || `https://picsum.photos/seed/${sceneIndex + 1}-end/720/1280`;
