...
2.  An "imgPrompt" field with a detailed image generation prompt that captures the essence of the scene, following the specified art style. The image prompt must have the following format:
  Scene Characters:
  - {Name}: Role: {role} / Description: {Character phisical description.}
  - Others charactars as needed.
  Scene Description: {A detailed description of the scene, including the environment, background elements, and any relevant details that set the scene.}
...
Generate the script. Each scene must be 10-15 seconds long, Max 2min all the video. Return only the JSON object.`,
});
